...
CQtImGuiRender* pC = nullptr;

CQtImGuiRender* CQtImGuiRender::instance()
{
	return pC;
}

CQtImGuiRender::CQtImGuiRender() : m_t(0)
{
	pC = this;
}

CQtImGuiRender::~CQtImGuiRender()
{

}

bool CQtImGuiRender::eventFilter(QObject* watched, QEvent* event)
{
	switch (event->type()) {
...
std::vector<ImVec2> vecVector;

void ImGuiLines::DrawLine(qreal x, qreal y)
{
	vecVector.push_back(ImVec2(x, y));
}

void ImGuiLines::Clear()
{
	vecVector.clear();
}

inline void DrawLines(ImDrawList* draw_list)
{
	static ImVec4 colf = ImVec4(0.2f, 0.2f, 0.2f, 1.0f);
	static float sz = 36.0f;
	static float f = 50.0f;

	ImGui::SliderFloat("size", &sz, 2.0f, 72.0f);
	ImGui::SliderFloat("thickness", &f, 1.0f, 50.0f);
	ImGui::ColorEdit4("Color", &colf.x);

	draw_list->AddPolyline(vecVector.data(), vecVector.size(), ImColor(colf.x, colf.y, colf.z, colf.w), true, false);
	//for (ImVec2 thisVec : vecVector) {}
}

void CQtImGuiRender::paint()
{
	newFrame();

	// 1. Show a simple window
	// Tip: if we don't call ImGui::Begin()/ImGui::End() the widgets appears in a window automatically called "Debug"
	ImVec2 lineVectors[32] = {};
	ImGui::SetNextWindowSize(ImGui::GetIO().DisplaySize);
	ImGui::SetNextWindowPos(ImVec2(0, 0));
	if (ImGui::Begin("Unknown window", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_AlwaysAutoResize)) {
...
void CQtImGuiRender::onMousePressedChange(QMouseEvent* event)
{
...
void CQtImGuiRender::onWheel(QWheelEvent* event)
{
	// 5 lines per unit
...
void CQtImGuiRender::onKeyPressRelease(QKeyEvent* event)
{
	ImGuiIO& io = ImGui::GetIO();
...
void CQtImGuiRender::renderDrawList(ImDrawData* draw_data)
{
...
bool CQtImGuiRender::createFontsTexture()
{
	// Build texture atlas
	ImGuiIO& io = ImGui::GetIO();
...
bool CQtImGuiRender::createDeviceObjects()
{
	// Backup GL state
...
bool CQtImGuiRender::initialize()
{
	...
	io.GetClipboardTextFn = [](void* user_data) {
		Q_UNUSED(user_data);
		instance()->g_currentClipboardText = QGuiApplication::clipboard()->text().toUtf8();
		return (const char*)instance()->g_currentClipboardText.data();
	};
...
CQtImGui::CQtImGui() : m_t(0), pRender(nullptr)
{
	connect(this, &QQuickItem::windowChanged, this, &CQtImGui::handleWindowChanged);
}

qreal CQtImGui::t() const { return m_t; }

void CQtImGui::setT(qreal t)
{
	if (t == m_t)
		return;
	m_t = t;
...
{
	if (!pRender) {
		pRender = new CQtImGuiRender();
		connect(window(), &QQuickWindow::afterRendering, pRender, &CQtImGuiRender::paint, Qt::DirectConnection);
	}

	pRender->setViewportSize(window()->size() * window()->devicePixelRatio());
	pRender->setT(m_t);
	pRender->setWindow(window());
}

void CQtImGui::cleanup()
{
	if (pRender) {
		delete pRender;
		pRender = nullptr;
...
void  CQtImGui::handleWindowChanged(QQuickWindow* win)
{
	if (win) {
		connect(win, &QQuickWindow::beforeSynchronizing, this, &CQtImGui::sync, Qt::DirectConnection);
		connect(win, &QQuickWindow::sceneGraphInvalidated, this, &CQtImGui::cleanup, Qt::DirectConnection);
